...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    @torch.no_grad()
    def generate_reasoning_tokens_verl(
        self,
        ...
        """
        Generate free-form reasoning tokens from multimodal context.
        This is used by rollout-side think-with-image logic to parse <tool> calls.
        """
        input_embeddings = self.get_input_embeddings()(input_ids)
        projected_patch_embeddings = self._process_vision_features(pixel_values, input_embeddings, use_film)

        if self.proprio_projector is not None and proprio is not None:
            proprio = torch.as_tensor(
                proprio, device=projected_patch_embeddings.device, dtype=projected_patch_embeddings.dtype
            )
            projected_patch_embeddings = self._process_proprio_features(
                projected_patch_embeddings, proprio, self.proprio_projector
            )
